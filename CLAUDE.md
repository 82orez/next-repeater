...
- **TTS 페이지**(`TtsClient.tsx`)는 Player와 독립적입니다. Zustand 스토어를 사용하지 않고 React `useState`로 로컬 상태만 관리합니다. API 라우트(`src/app/api/tts/route.ts`)가 OpenAI TTS API를 프록시하며, API 키는 `.env.local`의 `OPENAI_API_KEY`에 저장합니다. 생성된 오디오의 Object URL도 동일한 패턴으로 언마운트 시 해제합니다.
