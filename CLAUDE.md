...
- **STT**(`SttClient.tsx`)는 TTS와 대칭 구조(독립·`useState`만·`ConfirmDialog`). API 라우트(`src/app/api/stt/route.ts`)가 `request.formData()`로 파일 받아 OpenAI `audio.transcriptions` 프록시, 키 동일 재사용. 모델 화이트리스트 `gpt-4o-transcribe`/`whisper-1`(기본·가운데)/`gpt-4o-mini-transcribe`. **25MB 제한**(서버+클라이언트 이중 가드). **출력 형식 `text`/`srt`/`vtt`**(`format` 필드, 미지정 시 `text`) — 셋 다 문자열로 오므로 응답 처리는 동일. ⚠️ **`srt`/`vtt`는 whisper-1 전용**(gpt-4o-transcribe 계열은 `json`만 지원): 서버가 조합을 400으로 막고, UI는 TTS의 legacy 음성과 같은 방식으로 모델 버튼 비활성 + whisper-1 자동 전환(토스트). 결과는 textarea+복사+다운로드이며 **파일명·확장자·MIME·카드 제목은 생성 시점 스냅샷(`result.{text,format,fileName}`)만 읽는다** — 현재 state를 읽으면 받은 뒤 형식을 바꿨을 때 SRT 내용에 `.vtt`가 붙는다(`TtsClient`의 `resultMeta`와 같은 함정). `Player.tsx` 헤더에 STT/TTS 링크.
